...
#l1 Loss smooth
# def noise_estimation_loss(model, x0, t, e, b, beta=1.0):
#     a = (1-b).cumprod(dim=0).index_select(0, t).view(-1, 1, 1, 1)
#     x = x0[:, 3:, :, :] * a.sqrt() + e * (1.0 - a).sqrt()
#     output = model(torch.cat([x0[:, :3, :, :], x], dim=1), t.float())
#     loss_elementwise = F.smooth_l1_loss(output, e, beta=beta, reduction='none')
#     loss = loss_elementwise.sum(dim=(1, 2, 3)).mean(dim=0)
#     return loss 
