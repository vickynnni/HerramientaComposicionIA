...
#nuevo
def noise_estimation_loss(model, x0, t, e, b, freq_weight=0.3):
    # Pérdida estándar
    ...
    # Añadir pérdida ponderada por frecuencia (más énfasis en frecuencias bajas/altas)
    # Asumiendo que el eje vertical es frecuencia
    freq_weights = torch.linspace(1.0, freq_weight, x0.shape[2]).to(x0.device)
    freq_weights = freq_weights.view(1, 1, -1, 1).expand_as(output)
    freq_weighted_loss = ((e - output).square() * freq_weights).sum(dim=(1, 2, 3)).mean(dim=0)
    
    return mse_loss + freq_weighted_loss
