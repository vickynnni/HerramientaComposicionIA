...
def reconstruct_signal_griffin_lim(magnitude_spectrogram, fft_size, hopsamp, iterations):
    ...
    while n > 0:
        ...
        print(magnitude_spectrogram.shape, reconstruction_angle.shape)
        # Discard magnitude part of the reconstruction and use the supplied magnitude spectrogram instead.
        # proposal_spectrogram = magnitude_spectrogram*np.exp(1.0j*reconstruction_angle)
        # proposal_spectrogram = magnitude_spectrogram * np.exp(1.0j * reconstruction_angle[:, :1024])
        # Line 290 (from traceback, note the slice index :1025)
        proposal_spectrogram = magnitude_spectrogram * np.exp(1.0j * reconstruction_angle[:, :1040])
